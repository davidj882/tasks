...
class Breadcrumbs {
	 ...
	 * @return	void
	 */		
	function push($page, $href, $class_href = null)
	{
		...
		// push breadcrumb
		$this->breadcrumbs[$href] = array('page' => $page, 'href' => $href, 'class_href' => $class_href);
	}
	 ...
	 * @return	void
	 */		
	function unshift($page, $href, $class_href)
	{
		// no crumb provided
		if (!$page OR !$href) return;
		...
		// add at firts
		array_unshift($this->breadcrumbs, array('page' => $page, 'href' => $href, 'class_href' => '$class_href'));
	}
		...
		if ($this->breadcrumbs) {
			...
			foreach ($this->breadcrumbs as $key => $crumb) {
				...
				if (end($keys) == $key) {
					...
					//class="'..'"
					if (!empty($crumb['class_href'])) {
						
						$output .= $this->crumb_last_open . '' . '<i class="fa '.$crumb['class_href'].'"></i> ' . $crumb['page'] . '' . $this->crumb_close;
					...
					if (!empty($crumb['class_href'])) {
						...
						$output .= $this->crumb_open.'<a href="' . $crumb['href'] . '">' . '<i class="fa '.$crumb['class_href'].'"></i> ' . $crumb['page'] . '</a> '.$this->crumb_divider.$this->crumb_close;
					} else {
